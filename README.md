...
`label-tiles` is a small open source tool for _drawing labeled bounding boxes on map images_ from any [tile server](https://en.wikipedia.org/wiki/Tiled_web_map). It separates labeling from data storage, so you can label any raster data that can be served on a map.
